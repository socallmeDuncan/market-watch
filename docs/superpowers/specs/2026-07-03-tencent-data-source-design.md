...
腾讯实时返回示例（协创 `sz300857`）：

```
v_sz300857="51~协创数据~300857~300.41~293.59~293.59~173803~92752~81051~..."
                 ↑名称  ↑代码 ↑最新价 ↑昨收  ↑今开  ↑成交量(手)
字段索引(0基): 1=名称 2=代码 3=最新价 4=昨收 5=今开 6=成交量
              33=最高 34=最低 37=成交额(万) 38=换手率 39=市盈率 ...
```
...
腾讯 `~` 字段不全的部分（如总市值、60日涨跌幅等），对应列填 `None` / `pd.NA`，normalize.py 的 `_parse_optional_number` 已经能处理空值。
...
每条数据通过 `_market_watch_source` 列标记来源，最终写入 record 的 `source` 字段：
